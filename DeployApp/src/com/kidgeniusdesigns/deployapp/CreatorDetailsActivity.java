...
public class CreatorDetailsActivity extends FragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		actionBar.addTab(actionBar.newTab().setText("Event Details")
				.setTabListener(tabListener));
		actionBar.addTab(actionBar.newTab().setText("Attendees")
				.setTabListener(tabListener));
		actionBar.addTab(actionBar.newTab().setText("Invite Friends")
				.setTabListener(tabListener));
		...
		try {
			...
			mAttendeeTable.where().field("eventcode").eq(eventCode)
					.execute(new TableQueryCallback<Attendee>() {
						public void onCompleted(List<Attendee> result,
								...
								mDetailsTable.update(item,
										new TableOperationCallback<Events>() {
											public void onCompleted(
													...
													ServiceFilterResponse response) {
												if (exception == null) {
													Toast.makeText(getApplicationContext(), "Saved changes",
															Toast.LENGTH_LONG).show();
												}else{
